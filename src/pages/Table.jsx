...
const columns = [
  ...
  { key: 'setsAgainst', label: 'SA' },
  { key: 'setsDiff', label: 'SD' },
...
function computeStandings(division) {
  ...
  const stats = Object.fromEntries(players.map(p => [p, {
    name: p, played: 0, wins: 0, losses: 0, setsFor: 0, setsAgainst: 0, points: 0
  }]))

  for (const m of data.matches.filter(m => m.division === division && m.status === 'completed')) {
    ...
    for (const s of m.sets) {
      ...
      a.setsFor += s.A; a.setsAgainst += s.B
      b.setsFor += s.B; b.setsAgainst += s.A
    }
    a.played++; b.played++
    if (setsA > setsB) { a.wins++; b.losses++; a.points += 3 }
